...
function encontrarMaiorNumero(...parametro:number[]) {
    let maior = 0;
...
function verificarParOuImpar(x:number) {
    if (x % 2 == 0) {
        return true; 
    } else {
        return false; 
...
function retornarcaixaalta(palavras:string){
...
function inverterelementos(...variavel1: number[]){
    let novaelementos = "";
...
function receberporcentagem(valor:number, porcentagem:number){
    var resultado= 0;
    if(valor === 0){
        return 0;
    }
...
function calcularfatorial(numeros:number){
    let resultado=1;
